...
class FrequentPattern(Apriori):
    ...
    def find_freqsets(self, dataset):
        freqsets = {}
        self.ds_num = sum(dataset.values())
        self._find_freqsets(dataset, freqsets)
        freqsets_lst = [[] for x in range(self.max_rule_length)]
        support_counter = {}
    ...
    def _create_tree(self, dataset):
        ...
        for item, count in list(item_header.items()):
...
if __name__=='__main__':
    dataset = {frozenset('rzhjp'), frozenset('zyxwvuts'), frozenset('z'),
	           frozenset('rxnos'), frozenset('yrxzqtp'), frozenset('yzxeqstm')}
