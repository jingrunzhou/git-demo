...
# 加载数据集
def loadDataSet():
    dataSet3 = [line.split() for line in open("EpiData_test.dat", encoding='UTF-8').readlines()]
    return dataSet3
...
def main():
    ...
    # # 发现毒蘑菇的相似特性
    # # 得到全集的数据
    dataSet = [line.split() for line in open("EpiData5.dat",encoding='UTF-8').readlines()]
    L, supportData = apriori(dataSet, minSupport=0.3)
    # # 2表示毒蘑菇，1表示可食用的蘑菇
